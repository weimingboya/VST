...
from .create_act import get_activation
...
class BertSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # self.dg = 8
        # self.fc_gq = nn.Linear(hidden_size, 8*self.dg)
        # self.relu = nn.ReLU()
    ...
    def forward(self, hidden_states, attention_mask, history_states=None, ape=None, rpe=None):
        ...
        if rpe is not None:
            attention_scores = torch.log(torch.clamp(rpe, min=1e-6)) + attention_scores
            # gq = self.fc_gq(query).view(*query.shape[:2], 8, self.dg).permute(0, 2, 1, 3).unsqueeze(-1)  # (b_s, h, nq, d_k, 1)
            # geometric_bias = self.relu(torch.matmul(rpe, gq).squeeze(-1) / math.sqrt(self.dg))  # (b_s, h, nq, nk)
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, history_states=None, ape=None, rpe=None):
        self_output, attention_probs = self.self(input_tensor, attention_mask, history_states, ape, rpe)
        if ape is not None:
            input_tensor = input_tensor + ape
...
class BertXAttention(nn.Module):
    ...
    def forward(self, query, key, value, attention_mask, ape_query=None, ape_key=None, rpe=None):
        if ape_query is not None:
            query = query + ape_query
        if ape_key is not None:
            key = key + ape_key
...
class BertCrossAttention(nn.Module):
    ...
    def forward(self, query, key, value, attention_mask, q_attention_mask, ape_query=None, ape_key=None, rpe=None):
        x_output, attention_probs = self.self(query, key, value, attention_mask, ape_query, ape_key, rpe)
        if ape_query is not None:
            query = query + ape_query
