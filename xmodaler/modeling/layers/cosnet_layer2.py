...
class COSJointAttention(nn.Module):
    ...
    def forward(
        ...
        vo_mixed_key_layer, vo_mixed_value_layer = self.vo_attn(vo_feats).split(self.all_head_size, dim=2)
        ...
        vo_outputs2 = self.attn(mixed_query_layer, vo_mixed_key_layer, vo_mixed_value_layer, vo_attention_mask)
        vo_outputs = self.vo_proj(torch.cat([v_outputs, o_outputs], dim=-1))
        
        gate2 = self.gate_attn2(torch.cat([vo_outputs, vo_outputs2], dim=-1))
        gate2 = self.gate2(gate2)
        vo_outputs = gate2 * vo_outputs + (1 - gate2) * vo_outputs2
