...
class TransformerEncoderDecoder(BaseEncoderDecoder):
    ...
    def get_extended_attention_mask(self, batched_inputs):
        if kfg.TOKENS_MASKS not in batched_inputs:
            batched_inputs[kfg.TOKENS_MASKS] = torch.ones((batched_inputs[kfg.G_TOKENS_TYPE].size(0), self.max_seq_len)).cuda()
        ...
        if kfg.ATT_FEATS in batched_inputs:
            ...
            ret.update( {kfg.ATT_MASKS: vmasks, kfg.EXT_ATT_MASKS: ext_vmasks} )
