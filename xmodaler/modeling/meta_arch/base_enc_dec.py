...
from ..embedding import build_embeddings, add_embedding_config
...
class BaseEncoderDecoder(nn.Module, metaclass=ABCMeta):
    ...
    def add_config(cls, cfg, tmp_cfg):
        add_embedding_config(cfg, tmp_cfg)
    ...
    def preprocess_batch(self, batched_inputs):
        ...
        if kfg.POS_EMBED in batched_inputs[0]:
            pos_embs = [x[kfg.POS_EMBED] for x in batched_inputs]
            pos_embs = pad_tensor(pos_embs, padding_value=0, use_mask=False)
            ret.update( { kfg.POS_EMBED: pos_embs } )

        if kfg.CONTEXT in batched_inputs[0]:
            
            context = [x[kfg.CONTEXT] for x in batched_inputs]
            ctx_whole_f, ctx_five_f, ctx_five_p, ctx_nine_f, ctx_nine_p = list(zip(*context))
            ...
            ret.update( { kfg.CONTEXT: context} )

        if kfg.ALIGNS in batched_inputs[0]:
            aligns = [x[kfg.ALIGNS] for x in batched_inputs]
            aligns = pad_tensor(aligns, padding_value=0, use_mask=False) # DLCT: aligns
            ret.update( { kfg.ALIGNS: aligns } )
        ...
        if kfg.SEQ_PER_SAMPLE in batched_inputs[0]:
            ...
            if kfg.ATT_FEATS in batched_inputs[0]:
                batch_size, regions_num, feats_dim = vfeats.size()
                vfeats = vfeats.unsqueeze(1).expand(batch_size, repeat_num, regions_num, feats_dim)
                vfeats = vfeats.reshape(-1, regions_num, feats_dim)
                vmasks = vmasks.unsqueeze(1).expand(batch_size, repeat_num, regions_num)
                vmasks = vmasks.reshape(-1, regions_num)
                ret.update({ kfg.ATT_FEATS: vfeats, kfg.ATT_MASKS: vmasks })

            if kfg.ATT_FEATS_WO_MASK in batched_inputs[0]:
                batch_size, *dim_arr = vfeats_wo_mask.size()
                vfeats_wo_mask = vfeats_wo_mask.unsqueeze(1).expand(batch_size, repeat_num, *dim_arr)
                vfeats_wo_mask = vfeats_wo_mask.reshape(-1, *dim_arr)
                ret.update({ kfg.ATT_FEATS_WO_MASK: vfeats_wo_mask})

            if kfg.POS_EMBED in batched_inputs[0]:
                batch_size, *dim_arr = pos_embs.size()
                pos_embs = pos_embs.unsqueeze(1).expand(batch_size, repeat_num, *dim_arr)
                pos_embs = pos_embs.reshape(-1, *dim_arr)
                ret.update({ kfg.POS_EMBED: pos_embs})
            ...
            if kfg.CONTEXT in batched_inputs[0]:
                def expand(x):
                    if isinstance(x, dict):
                        ...
                        dim_arr = list(x.size())
                        dim_arr.insert(1,repeat_num)
                        x = x.unsqueeze(1).expand( *dim_arr)
                        x = x.reshape(-1, *dim_arr[2:])
                    return x
            ...
            if kfg.ATT_FEATS_LOC in batched_inputs[0]:
                ...
                vfeats_loc = vfeats_loc.unsqueeze(1).expand(batch_size, repeat_num, regions_num, vfeats_loc_dim)
                vfeats_loc = vfeats_loc.reshape(-1, regions_num, vfeats_loc_dim)
                ret.update({ kfg.ATT_FEATS_LOC: vfeats_loc })

            if kfg.ALIGNS in batched_inputs[0]:
                grids_num = dim_arr[0]
                aligns = aligns.unsqueeze(1).expand(batch_size, repeat_num, regions_num, grids_num)
                aligns = aligns.reshape(-1, regions_num, grids_num)
                ret.update({ kfg.ALIGNS: aligns })

            if kfg.RELATION in batched_inputs[0]:
                relation = relation.unsqueeze(1).expand(batch_size, repeat_num, regions_num, regions_num)
                relation = relation.reshape(-1, regions_num, regions_num)
                ret.update({ kfg.RELATION: relation })
