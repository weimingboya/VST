...
class BertSelfOutput(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # self.aoa = nn.Sequential(nn.Linear(4 * hidden_size, 2 * hidden_size), nn.GLU())
        # self.aoa_drop = nn.Dropout(hidden_dropout_prob)
    ...
    def forward(self, hidden_states, input_tensor):
        ...
        # hidden_states = self.aoa(torch.cat([hidden_states, input_tensor], -1))
...
class BertGenerationLayer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # self.fc_alpha4 = nn.Linear(512*2, 512)
        ...
        # self.aoa3 = nn.Sequential(nn.Linear(2 * d_model, 2 * d_model), nn.GLU())
    ...
    def from_config(cls, cfg):
        return {
            ...
            # "bert_comm_cross_attention": BertXAttention(cfg),
    ...
    def forward(self, lang_feats, visn_feats, text_feats, lang_attention_mask=None, v_attention_mask=None, t_history_states=None):
        ...
        # visn_feats = self.linear1(visn_feats)
        # text_feats = self.linear2(text_feats)
        # cat_feats = self.linear3(cat_feats)
        
        # vt_feats = torch.cat([visn_feats, text_feats], dim=-1)
        # vt_feats = self.vt_linear(vt_feats)
        
        # final_feats = self.cross_output(vt_feats, lang_feats)
        ...
        # alpha1 = self.fc_alpha1(torch.cat([lang_feats, vt_feats], -1))
        # alpha2 = self.fc_alpha2(torch.cat([lang_feats, cat_feats], -1))
        ...
        # final_feats = (visn_feats * alpha1 + text_feats * alpha2 + cat_feats * alpha3) / np.sqrt(3)
        
        # final_feats = (vt_feats * alpha1 + cat_feats * alpha2) / np.sqrt(2)
        # final_feats = torch.cat([visn_feats, text_feats, cat_feats], dim=-1)
        # final_feats = torch.cat([mix_feats, cat_feats], dim=-1)
        
        # visn_feats = self.aoa1(torch.cat([visn_feats, lang_feats], -1))
        # text_feats = self.aoa2(torch.cat([text_feats, lang_feats], -1))
        # cat_feats = self.aoa3(torch.cat([cat_feats, lang_feats], -1))
        
        # final_feats = torch.cat([visn_feats, text_feats, cat_feats], dim=-1)
        final_feats = torch.cat([visn_feats, text_feats], dim=-1)
        # final_feats = visn_feats + text_feats + cat_feats
        
        # final_feats = self.aoa1(torch.cat([final_feats, lang_feats], -1))
        ...
        # alpha3 = self.fc_alpha3(torch.cat([mix_feats, lang_feats, cat_feats], -1))
        # alpha3 = torch.sigmoid(alpha3)
        # final_feats = mix_feats * alpha3 + cat_feats * (1 - alpha3)
        ...
        # final_feats = mix_feats * alpha3 + cat_feats * alpha4

        # intermediate_output = self.intermediate3(final_feats)
        # final_feats = self.output3(intermediate_output, final_feats)
