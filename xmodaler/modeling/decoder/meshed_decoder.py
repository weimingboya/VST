...
class MeshedDecoderLayer(nn.Module):
    ...
    def forward(self, input, enc_output, mask_self_att, mask_enc_att, history_states=None):
        ...
        enc_att1 = self.enc_att(self_att, keys=enc_output[:, :64], values=enc_output[:, :64], attention_mask=mask_enc_att)
        enc_att2 = self.enc_att(self_att, keys=enc_output[:, 64:64+16], values=enc_output[:, 64:64+16], attention_mask=mask_enc_att)
        enc_att3 = self.enc_att(self_att, keys=enc_output[:, 64+16:64+16+4], values=enc_output[:, 64+16:64+16+4], attention_mask=mask_enc_att)
        alpha1 = torch.sigmoid(self.fc_alpha[0](torch.cat([self_att, enc_att1], -1)))
        alpha2 = torch.sigmoid(self.fc_alpha[1](torch.cat([self_att, enc_att2], -1)))
        alpha3 = torch.sigmoid(self.fc_alpha[2](torch.cat([self_att, enc_att3], -1)))


        enc_att = enc_att1 * alpha1 +  enc_att2 * alpha2 + enc_att3 * alpha3
